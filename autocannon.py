...
RUN_DIR = os.environ.get("AUTOCANNON_RUN", "./run")
CAP = int(os.environ.get("AUTOCANNON_CAP", "8"))          # concurrent troopers
DOMAIN = os.environ.get("AUTOCANNON_DOMAIN", "target.local")

_lock = threading.Lock()
# Two-channel feed (mailbox 2026-08-21-gunbelt-10-MANAGER-CHANNEL-SCRUB-PATCH):
#   MGR_FEED (canonical autocannon.jsonl) — manager/Opus-safe: labels + fact KEYS + counts only.
#   OPS_FEED (autocannon.ops.jsonl) — operator/browser: objectives + evidence + fact values.
MGR_FEED = "autocannon.jsonl"
OPS_FEED = "autocannon.ops.jsonl"
...
def emit(kind, tool, text, lane="", phase=""):
    """Already-neutral rows -> BOTH feeds (BS2 War Room schema)."""
    r = _row(kind, tool, text, lane, phase); _write(MGR_FEED, r); _write(OPS_FEED, r)
    ...
    L += [
      ...
      dict(id="crack-creds", phase="cred", needs={"hash"}, gives="cred", tool="hashcat",
           objective="You have one or more WordPress phpass ($P$) hashes. DERIVE the plaintext "
                     ...
                     "has no GPU backend, fall back to: python3 ./"
...
class Autocannon:
    ...
    def fire(self, lane):
        ...
        if rv is not None:
            ...
            src = "trooper"
            emit_split("cmd", lane["tool"], f"▶ {lane['id']} firing", f"▶ {lane['id']}: {obj[:120]}", lane["id"], lane["phase"])
            v = self.tp.fire({"id": lane["id"], "target": self.target, "objective": obj})
        save_raw(lane["id"], v.get("output", ""))
        ok = bool(v.get("success"))
        emit_split("out", lane["tool"], f"{'✔ HIT' if ok else '✘ miss'} [{src}]",
                   f"{'✔ HIT' if ok else '✘ miss'} [{src}] — {v.get('evidence','')}",
                   lane["id"], lane["phase"])
        for fact in v.get("facts", []):
            _k = fact.split("=", 1)[0].strip() if "=" in fact else fact
            emit_split("fact", lane["tool"], f"⚑ {_k}", f"⚑ {fact}", lane["id"], lane["phase"])
        return lane, v
    ...
    def run(self, max_rounds=10):
        os.makedirs(RUN_DIR, exist_ok=True)
        open(os.path.join(RUN_DIR, MGR_FEED), "w").close()   # fresh feed (manager)
        open(os.path.join(RUN_DIR, OPS_FEED), "w").close()   # fresh feed (operator)
