...
dominates. Backends, fastest-first:
  ...
  your-host sec-specialist (3070): TROOPER_BASE=http://YOUR-LOCAL-LLM-HOST:11434/v1
  ...
  DeepSeek (when funded):       TROOPER_BASE=https://api.deepseek.com    MODEL=deepseek-v4-pro
                                TROOPER_KEY_FILE=/path/to/your/.ds_key
...
# Opt-in per run via TROOPER_REDZONE="10.10.14." (a dotted PREFIX). Only that prefix is
# exempted from the fleet-LAN deny; every other YOUR.LAN.x — your-host/your-host/the LAN — stays denied,
...
SYS_TMPL = ("/no_think\n"     # qwen3: skip the chain-of-thought block — 0.84s vs 9.5s/turn, and it
       ...
       "how you run out of turns with nothing proven. To see what is available: `python3 ./recipes.py --list`. To "
       "run one against the target: `TROOPER_EXEC_SSH= python3 ./recipes.py "
...
def _salvage_facts(source, target=""):
    ...
    for app in ("gitlab", "wordpress", "drupal", "osticket"):
        if re.search(app, joined, re.I):
            ...
            # VERSION (run 746 step 1: drupal/osticket/wordpress all got version '10.129.87')
            add(f"app={app}")
